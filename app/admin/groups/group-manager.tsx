...
import type { MemberGroupSummary } from "@/lib/member-groups";
...
export function GroupManager({
  ...
  const shown = useMemo(() => {
    ...
    return groups.filter((group) =>
      [group.name, group.description, ...group.memberIds.map(named)]
        .join(" ")
        .toLowerCase()
        .includes(needle)
    );
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div className="admin-list-meta">
                  {group.memberIds.length === 0
                    ? "nobody in it yet"
                    : group.memberIds
                        .map((id) => nameById.get(id) ?? "an account that has gone")
                        .join(", ")}
                </div>
              </div>
...
function GroupDialog({
  ...
  const available = members.filter((member) => !memberIds.includes(member._id));
        ...
        >
          <div className="style-modal-form">
            ...
            <div className="style-modal-body">
              ...
              <div className="field" style={{ marginTop: "0.875rem" }}>
                <span className="field-label">Members ({memberIds.length})</span>
                ...
                />
                <span className="help-text">
                  A group can be saved empty and filled in later.
                </span>
              </div>
            </div>
